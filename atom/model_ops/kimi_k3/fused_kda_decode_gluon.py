...
def _fused_kda_decode_gluon_kernel(
    ...
    for i_t in range(seq_T):
        ...
        b_q = b_x_q * gl.load(conv_weight_ptr + cw_q_base + o_k * stride_cw_ch + (W - 1) * stride_cw_width).to(tl.float32)
        for j in gl.static_range(W - 1):
            cs = gl.load(p_csq + j * stride_cs_pos).to(tl.float32)
            cw = gl.load(conv_weight_ptr + cw_q_base + o_k * stride_cw_ch + j * stride_cw_width).to(tl.float32)
            b_q = b_q + cs * cw
        ...
        b_k = b_x_k * gl.load(conv_weight_ptr + cw_k_base + o_k * stride_cw_ch + (W - 1) * stride_cw_width).to(tl.float32)
        for j in gl.static_range(W - 1):
            cs = gl.load(p_csk + j * stride_cs_pos).to(tl.float32)
            cw = gl.load(conv_weight_ptr + cw_k_base + o_k * stride_cw_ch + j * stride_cw_width).to(tl.float32)
            b_k = b_k + cs * cw
        ...
        b_v_full = b_x_v * gl.load(conv_weight_ptr + cw_v_base + o_v_full * stride_cw_ch + (W - 1) * stride_cw_width).to(tl.float32)
        for j in gl.static_range(W - 1):
            cs = gl.load(p_csv + j * stride_cs_pos).to(tl.float32)
            cw = gl.load(conv_weight_ptr + cw_v_base + o_v_full * stride_cw_ch + j * stride_cw_width).to(tl.float32)
        ...
        b_beta = tl.sigmoid(gl.load(beta_ptr + tok * stride_beta_tok + i_h).to(tl.float32))
        ...
        for i_c in gl.static_range(N_CHUNKS):
            ...
            b_og = gl.load(out_gate_ptr + tok * stride_og_tok + i_h * V + o_v,
